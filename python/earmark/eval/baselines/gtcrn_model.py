...
Vendored from the official implementation, https://github.com/Xiaobin-Rong/gtcrn, file
gtcrn.py at commit 502ebfab64da7c4a9af78dcb9c6ceef1ebb01c73.
Copyright (c) 2024 Rong Xiaobin. MIT License; the full text is in GTCRN_LICENSE.txt next to
this file and must travel with it.

Modifications (behaviour-preserving, so the official checkpoints load unchanged):
  1. The einops dependency is removed: GTConvBlock.shuffle uses an equivalent reshape of the
     contiguous (B, C, 2, T, F) tensor instead of rearrange('b c g t f -> b (c g) t f').
  2. GRNN.forward tests `h is None` instead of `h== None`.
  3. The `__main__` demo block (ptflops complexity count and a causality check written for
     the deprecated torch.stft(return_complex=False) API) is dropped.
Everything else is the upstream code, left unformatted for easy diffing against it.
"""
import torch
import numpy as np
...
class GTConvBlock(nn.Module):
    ...
    def shuffle(self, x1, x2):
        ...
        # Upstream: rearrange(x, 'b c g t f -> b (c g) t f'); identical for a contiguous tensor.
        x = x.reshape(x.shape[0], x.shape[1] * x.shape[2], x.shape[3], x.shape[4])  # (B,2C,T,F)
        return x
