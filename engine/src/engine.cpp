// The C API (include/earmark.h) over the week-1 signal path.
//
...
// One 16 kHz hop through the model's signal path. The network is not wired yet: the
// features are computed (and the normalisation state advances) but the gains stay at 1
// and there is no deep filter, so out is the input delayed by one hop.
...
const char* em_build_info(void) {
  ...
  return "earmark-engine abi=1 contract=" EARMARK_CONTRACT_HASH " matvec=wasm-simd128 network=not-wired";
#else
  return "earmark-engine abi=1 contract=" EARMARK_CONTRACT_HASH " matvec=scalar network=not-wired";
#endif
}
...
em_engine* em_create(const uint8_t* blob, size_t blob_bytes, const char* manifest, size_t manifest_bytes,
  ...
  *result = EM_OK;
  ...
  e->state_bytes = sizeof(earmark::StreamState) +
                   (history_floats + model_in_capacity + device_out_capacity + kEmbeddingDim) * sizeof(float);
...
em_status em_set_embedding(em_engine* engine, const float* embedding) {
  ...
  if (embedding == nullptr) {
    std::memcpy(engine->embedding, engine->null_embedding, kEmbeddingDim * sizeof(float));
    engine->personal = false;
    return EM_OK;
  }
  if (!all_finite(embedding, kEmbeddingDim)) return EM_ERR_ARGUMENT;
  std::memcpy(engine->embedding, embedding, kEmbeddingDim * sizeof(float));
  engine->personal = true;
  return EM_OK;
}
...
size_t em_arena_bytes(const em_engine* engine) { return engine != nullptr ? engine->arena.capacity() : 0; }
